...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt(argc, argv, "h:u:p:")) != -1) {
        ...
        case 'u':
            // Throw error if cant be converted to ID
...
int Client::connectTo()
{
    ...
    coord_stub = std::unique_ptr<Coordinator_Service::Stub>(Coordinator_Service::NewStub(
        grpc::CreateChannel(
            login_info, grpc::InsecureChannelCredentials())));

    coord_stub->HandleClient(&context, request, &reply);

    std::string server_port = reply.msg();
    std::cout << "Obtained port: " << server_port << std::endl;

    login_info = hostname + ":" + server_port;
    stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
        grpc::CreateChannel(
            login_info, grpc::InsecureChannelCredentials())));

    // Can make coord_stub nullptr? -> Remove active connection
